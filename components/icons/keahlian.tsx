...
import {
  SiHtml5, SiCss, SiBootstrap, SiTailwindcss, SiJavascript,
  SiTypescript, SiReact, SiVuedotjs, SiAstro, SiNextdotjs,
  SiNodedotjs, SiExpress, SiGo, SiPhp, SiLaravel,
  SiKotlin, SiPostgresql, SiMysql, SiFirebase, SiSupabase,
  SiDocker, SiNpm, SiYarn, SiPrisma, SiRedux, SiFramer,
  SiVite, SiGithub,
  SiShadcnui,
  SiAxios,
  SiZod,
...
const SIZE_MAP: Record<IconSize, { wrapper: number; icon: number; fontSize: number }> = {
...
// ─── TechIcon ─────────────────────────────────────────────────────
export function TechIcon({ name, size = "md", customIcon, customBg, customLabel }: TechIconProps) {
  ...
  return (
    <div
      style={{ position: "relative", width: wrapper, height: wrapper, perspective: "380px", display: "inline-block", cursor: "pointer" }}
    ...
    >
      {/* Back layer */}
      <span style={{
        position: "absolute", inset: 0, borderRadius: "38%", background: bg,
        transformOrigin: "bottom right",
        transform: hovered ? "rotate(25deg) translate3d(-6px,-6px,6px)" : "rotate(15deg)",
      ...
      <span style={{
        ...
        {IconComponent
          ...
          : <span style={{ fontSize: iconSize * 0.6, color: iconColor, fontWeight: 700 }}>{label?.slice(0, 2).toUpperCase()}</span>
        }
      </span>
      ...
      <span style={{
        ...
        opacity: hovered ? 1 : 0,
        whiteSpace: "nowrap", fontSize, color: "#e2e8f0",
        fontWeight: 500, pointerEvents: "none",
        transition: "opacity 0.3s ease, transform 0.3s cubic-bezier(0.83,0,0.17,1)",
      }}>
        {label}
      </span>
    </div>
...
// ─── TechIconGroup ────────────────────────────────────────────────
export function TechIconGroup({ icons = [], size = "md", gap = 12 }: TechIconGroupProps) {
  return (
